"""
Scraper for ADA (Australian Data Archive) - Using OAI-PMH protocol.
Note: ADA blocks API access in robots.txt, so we use OAI-PMH instead.
"""
import requests
import time
...
class ADAScraper(BaseScraper):
    """Scraper for Australian Data Archive (ADA) using OAI-PMH protocol."""
    ...
    def __init__(self, config_path: str = "config/qda_extensions.json"):
        ...
        self.oai_base = "https://dataverse.ada.edu.au/oai"
    ...
    def search(
        ...
        **kwargs
        ...
        """
        Search ADA for QDA files using OAI-PMH.
        ...
        print(f"Searching ADA via OAI-PMH for: '{query}'...")
        ...
        while total_fetched < max_results and total_checked < 1000:  # Limit to 1000 records
            try:
                if resumption_token:
                    params = {
                        'verb': 'ListRecords',
                ...
                for record in records:
                    ...
                    if metadata_elem is not None:
                        ...
                        # Check if it matches query and is QDA-related
                        if self._matches_query(file_meta, query):
                            self.results.append(file_meta)
                            total_fetched += 1
                            print(f"Found QDA-related record: {file_meta.get('project_title', '')[:50]}...")
                ...
                print(f"Error fetching from ADA: {e}")
                break
            except ET.ParseError as e:
                print(f"Error parsing XML from ADA: {e}")
                break

        print(f"Checked {total_checked} records, found {total_fetched} QDA-related results")
        return self.results
    ...
    def _extract_metadata(self, record: ET.Element, ns: Dict[str, str]) -> Dict[str, Any]:
        ...
        return self.normalize_metadata({
            'filename': f"{identifier.split(':')[-1]}.dataset",
            'file_extension': '',
            'download_url': source_url,
            'source_repository': 'ADA - Australian Data Archive',
    ...
    def _matches_query(self, metadata: Dict[str, Any], query: Optional[str]) -> bool:
        """Check if metadata matches the search query."""
        if not query:
            # If no query, check for QDA-related keywords
            return self._is_qda_related(metadata)

        # Check if query appears in title, description, or keywords
        text_to_check = (
        ...
        # Check if it's a QDA extension search
        if query_lower in [ext.lstrip('.') for ext in self.qda_extensions]:
            # For extension searches, look for the extension or related software
            return query_lower in text_to_check or self._is_qda_related(metadata)

        # For other queries, just check if query appears in text
        return query_lower in text_to_check

    def _is_qda_related(self, metadata: Dict[str, Any]) -> bool:
        """Check if metadata suggests QDA-related data."""
        qda_keywords = [
            ...
            'nvivo', 'maxqda', 'atlas.ti', 'dedoose', 'qdpx', 'mqda',
            'coding', 'thematic analysis', 'qualitative data'
        ]
        ...
        return any(keyword in text_to_check for keyword in qda_keywords)
