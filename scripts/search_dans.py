#!/usr/bin/env python3
"""Search DANS repository with all queries."""
...
import time
import json
from src.scrapers import DANSScraper
...
def main():
    ...
    cursor.execute("SELECT COUNT(*) FROM files WHERE source_repository LIKE '%DANS%'")
    start_repo = cursor.fetchone()[0]
    print(f"[DB] Total files in database: {start_count}")
    print(f"[DB] DANS files: {start_repo}")
    ...
    print(f"[DONE] DANS files: {start_repo} -> {end_repo} (+{end_repo - start_repo})")
