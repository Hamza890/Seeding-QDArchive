#!/usr/bin/env python3
"""Search Qualidata repository with all queries."""
...
import time
import json
from src.scrapers import QualidataScraper
...
def main():
    ...
    cursor.execute("SELECT COUNT(*) FROM files WHERE source_repository LIKE '%Qualidata%'")
    start_repo = cursor.fetchone()[0]
    print(f"[DB] Total files in database: {start_count}")
    print(f"[DB] Qualidata files: {start_repo}")
    ...
    print(f"[DONE] Qualidata files: {start_repo} -> {end_repo} (+{end_repo - start_repo})")
