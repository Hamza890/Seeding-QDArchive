#!/usr/bin/env python3
"""Search Qualiservice repository with all queries."""
...
import time
import json
from src.scrapers import QualiserviceScraper
...
def main():
    ...
    cursor.execute("SELECT COUNT(*) FROM files WHERE source_repository LIKE '%Qualiservice%'")
    start_repo = cursor.fetchone()[0]
    print(f"[DB] Total files in database: {start_count}")
    print(f"[DB] Qualiservice files: {start_repo}")
    ...
    print(f"[DONE] Qualiservice files: {start_repo} -> {end_repo} (+{end_repo - start_repo})")
