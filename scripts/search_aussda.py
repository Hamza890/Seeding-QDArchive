#!/usr/bin/env python3
"""Search AUSSDA repository with all queries."""
...
import time
import json
from src.scrapers import AUSSDAScraper
...
def main():
    ...
    cursor.execute("SELECT COUNT(*) FROM files WHERE source_repository LIKE '%AUSSDA%'")
    start_repo = cursor.fetchone()[0]
    print(f"[DB] Total files in database: {start_count}")
    print(f"[DB] AUSSDA files: {start_repo}")
    ...
    print(f"[DONE] AUSSDA files: {start_repo} -> {end_repo} (+{end_repo - start_repo})")
