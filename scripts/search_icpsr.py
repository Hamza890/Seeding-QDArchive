#!/usr/bin/env python3
"""Search ICPSR repository with all queries."""
...
import time
import json
from src.scrapers import ICSPRScraper
...
def main():
    ...
    cursor.execute("SELECT COUNT(*) FROM files WHERE source_repository LIKE '%ICPSR%'")
    start_repo = cursor.fetchone()[0]
    print(f"[DB] Total files in database: {start_count}")
    print(f"[DB] ICPSR files: {start_repo}")
    ...
    print(f"[DONE] ICPSR files: {start_repo} -> {end_repo} (+{end_repo - start_repo})")
