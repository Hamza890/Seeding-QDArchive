#!/usr/bin/env python3
"""Search Harvard Dataverse repository with all queries."""
...
import time
import json
from src.scrapers import HarvardDataverseScraper
...
def main():
    print("="*80)
    print("SEARCHING: Harvard Dataverse")
    print("="*80)
    ...
    cursor.execute("SELECT COUNT(*) FROM files WHERE source_repository LIKE '%Harvard%'")
    start_repo = cursor.fetchone()[0]
    print(f"[DB] Total files in database: {start_count}")
    print(f"[DB] Harvard files: {start_repo}")
    
    queries = load_all_queries()
    print(f"[INFO] Total queries: {len(queries)}")
    print(f"[INFO] Delay: 3.0s per query")
    print(f"[INFO] Estimated time: ~{len(queries) * 3 / 60:.1f} minutes")
    ...
    print(f"[DONE] Harvard files: {start_repo} -> {end_repo} (+{end_repo - start_repo})")
