#!/usr/bin/env python3
"""Search Dryad repository with all queries."""
...
import time
import json
from src.scrapers import DryadScraper
...
def main():
    ...
    cursor.execute("SELECT COUNT(*) FROM files WHERE source_repository LIKE '%Dryad%'")
    start_repo = cursor.fetchone()[0]
    print(f"[DB] Total files in database: {start_count}")
    print(f"[DB] Dryad files: {start_repo}")
    ...
    print(f"[DONE] Dryad files: {start_repo} -> {end_repo} (+{end_repo - start_repo})")
