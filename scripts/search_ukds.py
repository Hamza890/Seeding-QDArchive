#!/usr/bin/env python3
"""Search UK Data Service repository with all queries."""
...
import time
import json
from src.scrapers import UKDataServiceScraper
...
def main():
    print("="*80)
    print("SEARCHING: UK Data Service")
    print("="*80)
    ...
    cursor.execute("SELECT COUNT(*) FROM files WHERE source_repository LIKE '%UK%'")
    start_repo = cursor.fetchone()[0]
    print(f"[DB] Total files in database: {start_count}")
    print(f"[DB] UK Data Service files: {start_repo}")
    ...
    print(f"[DONE] UK Data Service files: {start_repo} -> {end_repo} (+{end_repo - start_repo})")
