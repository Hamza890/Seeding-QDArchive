#!/usr/bin/env python3
"""Search Syracuse QDR repository with all queries."""
...
import time
import json
from src.scrapers import SyracuseQDRScraper
...
def main():
    ...
    cursor.execute("SELECT COUNT(*) FROM files WHERE source_repository LIKE '%Syracuse%'")
    start_repo = cursor.fetchone()[0]
    print(f"[DB] Total files in database: {start_count}")
    print(f"[DB] Syracuse QDR files: {start_repo}")
    
    queries = load_all_queries()
    print(f"[INFO] Total queries: {len(queries)}")
    print(f"[INFO] Delay: 1.0s per query")
    
    scraper = SyracuseQDRScraper()
    results = []
    seen_ids = set()
    ...
    print(f"[DONE] Syracuse files: {start_repo} -> {end_repo} (+{end_repo - start_repo})")
