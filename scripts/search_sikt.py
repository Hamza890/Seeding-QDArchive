#!/usr/bin/env python3
"""Search Sikt Norway repository with all queries."""
...
import time
import json
from src.scrapers import SiktScraper
...
def main():
    ...
    cursor.execute("SELECT COUNT(*) FROM files WHERE source_repository LIKE '%Sikt%'")
    start_repo = cursor.fetchone()[0]
    print(f"[DB] Total files in database: {start_count}")
    print(f"[DB] Sikt files: {start_repo}")
    
    queries = load_all_queries()
    print(f"[INFO] Total queries: {len(queries)}")
    print(f"[INFO] Delay: 1.5s per query")
    print(f"[INFO] Estimated time: ~{len(queries) * 1.5 / 60:.1f} minutes")
    ...
    print(f"[DONE] Sikt files: {start_repo} -> {end_repo} (+{end_repo - start_repo})")
