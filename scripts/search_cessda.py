#!/usr/bin/env python3
"""Search CESSDA repository with all queries."""
...
import time
import json
from src.scrapers import CESSDAScraper
...
def main():
    ...
    cursor.execute("SELECT COUNT(*) FROM files WHERE source_repository LIKE '%CESSDA%'")
    start_repo = cursor.fetchone()[0]
    print(f"[DB] Total files in database: {start_count}")
    print(f"[DB] CESSDA files: {start_repo}")
    
    queries = load_all_queries()
    print(f"[INFO] Total queries: {len(queries)}")
    print(f"[INFO] Delay: 2.5s per query")
    print(f"[INFO] Estimated time: ~{len(queries) * 2.5 / 60:.1f} minutes")
    ...
    print(f"[DONE] CESSDA files: {start_repo} -> {end_repo} (+{end_repo - start_repo})")
