#!/usr/bin/env python3
"""Search QualiBi repository with all queries."""
...
import time
import json
from src.scrapers import QualiBiScraper
...
def main():
    ...
    cursor.execute("SELECT COUNT(*) FROM files WHERE source_repository LIKE '%QualiBi%'")
    start_repo = cursor.fetchone()[0]
    print(f"[DB] Total files in database: {start_count}")
    print(f"[DB] QualiBi files: {start_repo}")
    ...
    print(f"[DONE] QualiBi files: {start_repo} -> {end_repo} (+{end_repo - start_repo})")
