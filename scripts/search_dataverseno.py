#!/usr/bin/env python3
"""Search DataverseNO repository with all queries."""
...
import time
import json
from src.scrapers import DataverseScraper
...
def main():
    ...
    cursor.execute("SELECT COUNT(*) FROM files WHERE source_repository LIKE '%Dataverse%'")
    start_repo = cursor.fetchone()[0]
    print(f"[DB] Total files in database: {start_count}")
    print(f"[DB] DataverseNO files: {start_repo}")
    
    queries = load_all_queries()
    print(f"[INFO] Total queries: {len(queries)}")
    print(f"[INFO] Delay: 2.0s per query")
    print(f"[INFO] Estimated time: ~{len(queries) * 2 / 60:.1f} minutes")
    
    scraper = DataverseScraper()
    results = []
    seen_ids = set()
    ...
    print(f"[DONE] DataverseNO files: {start_repo} -> {end_repo} (+{end_repo - start_repo})")
