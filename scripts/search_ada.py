#!/usr/bin/env python3
"""Search ADA Australian repository with all queries."""
...
import time
import json
from src.scrapers import ADAScraper
...
def main():
    print("="*80)
    print("SEARCHING: ADA Australian")
    print("="*80)
    ...
    cursor.execute("SELECT COUNT(*) FROM files WHERE source_repository LIKE '%ADA%'")
    start_repo = cursor.fetchone()[0]
    print(f"[DB] Total files in database: {start_count}")
    print(f"[DB] ADA Australian files: {start_repo}")
    ...
    print(f"[DONE] ADA Australian files: {start_repo} -> {end_repo} (+{end_repo - start_repo})")
