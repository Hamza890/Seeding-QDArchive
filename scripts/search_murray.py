#!/usr/bin/env python3
"""Search Murray Archive repository with all queries."""
...
import time
import json
from src.scrapers import MurrayScraper
...
def main():
    print("="*80)
    print("SEARCHING: Murray Archive")
    print("="*80)
    ...
    cursor.execute("SELECT COUNT(*) FROM files WHERE source_repository LIKE '%Murray%'")
    start_repo = cursor.fetchone()[0]
    print(f"[DB] Total files in database: {start_count}")
    print(f"[DB] Murray Archive files: {start_repo}")
    ...
    print(f"[DONE] Murray Archive files: {start_repo} -> {end_repo} (+{end_repo - start_repo})")
