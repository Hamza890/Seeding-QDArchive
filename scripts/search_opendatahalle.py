#!/usr/bin/env python3
"""Search Open Data Halle repository with all queries."""
...
import time
import json
from src.scrapers import OpenDataHalleScraper
...
def main():
    print("="*80)
    print("SEARCHING: Open Data Halle")
    print("="*80)
    ...
    cursor.execute("SELECT COUNT(*) FROM files WHERE source_repository LIKE '%Open%'")
    start_repo = cursor.fetchone()[0]
    print(f"[DB] Total files in database: {start_count}")
    print(f"[DB] Open Data Halle files: {start_repo}")
    ...
    print(f"[DONE] Open Data Halle files: {start_repo} -> {end_repo} (+{end_repo - start_repo})")
