#!/usr/bin/env python3
"""Search FSD Finland repository with all queries."""
...
import time
import json
from src.scrapers import FSDScraper
...
def main():
    ...
    cursor.execute("SELECT COUNT(*) FROM files WHERE source_repository LIKE '%FSD%'")
    start_repo = cursor.fetchone()[0]
    print(f"[DB] Total files in database: {start_count}")
    print(f"[DB] FSD Finland files: {start_repo}")
    ...
    print(f"[DONE] FSD Finland files: {start_repo} -> {end_repo} (+{end_repo - start_repo})")
