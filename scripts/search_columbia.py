#!/usr/bin/env python3
"""Search Columbia Oral History repository with all queries."""
...
import time
import json
from src.scrapers import ColumbiaOralHistoryScraper
...
def main():
    print("="*80)
    print("SEARCHING: Columbia Oral History")
    print("="*80)
    ...
    cursor.execute("SELECT COUNT(*) FROM files WHERE source_repository LIKE '%Columbia%'")
    start_repo = cursor.fetchone()[0]
    print(f"[DB] Total files in database: {start_count}")
    print(f"[DB] Columbia Oral History files: {start_repo}")
    ...
    print(f"[DONE] Columbia Oral History files: {start_repo} -> {end_repo} (+{end_repo - start_repo})")
