#!/usr/bin/env python3
"""Search IHSN repository with all queries."""
...
import time
import json
from src.scrapers import IHSNScraper
...
def main():
    ...
    cursor.execute("SELECT COUNT(*) FROM files WHERE source_repository LIKE '%IHSN%'")
    start_repo = cursor.fetchone()[0]
    print(f"[DB] Total files in database: {start_count}")
    print(f"[DB] IHSN files: {start_repo}")
    ...
    print(f"[DONE] IHSN files: {start_repo} -> {end_repo} (+{end_repo - start_repo})")
